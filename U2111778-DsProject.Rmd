...
- Evaluating model performance using metrics such as accuracy and variability.
...
We also want to consider whether our independent variables are statistically significant in predicting stars. To do this we consider two linear metrics, correlation and T-tests, and while these are both linear indicators, they nevertheless contribute insight to the project. 
...
A final point to note is that there is a large range in ‘afinn_score’ and ‘bing_score’, however after investigation, maintaining outliers produces a higher accuracy and therefore this is left as is. Additionally, the analysis indicated no relationship between review length and sentiment score. 
...
Demonstrated is the result of pruning at a low-cost complexity, which while it does improve the accuracy and MSE of the model, the issue of overfitting has not been solved, and the model has lost its interpretability. Increasing the cost complexity of the model has little effect as it merely reduces the performance and returns to the previous model.

Both of these cases have illustrated the fact that classification trees are not robust, perturbations in the data have resulted in a big effect on the final tree and ultimately MSE. In this case, this is exacerbated through unbalanced classes. 
...
This model does further reduce MSE, however does not improve the predictability of the result. We now seek a model which maintains the low MSE of linear models whilst keeping high predictability.

#### Random Forest 
To increase predictability whilst maintaining a low MSE we return to a classification model, however, to reduce the bias introduced within the model, we employ a random forest methodology which reduces both variance and bias and ultimately reduces MSE.

Here we can see that random forest achieves the highest predictability, whilst maintaining a lower MSEs. This is because of the nature of random forests it overcomes the impact of imbalanced datasets. 
...
We have seen how the different models have a trade-off between variance, bias and accuracy through overfitting or underfitting the data. The distribution of the data has posed significant challenges to classification models, the categorical nature of stars posed challenges for continuous linear models but ultimately an approach which randomly selects predictors 
